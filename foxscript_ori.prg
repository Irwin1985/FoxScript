...
Define Class FoxScript As Custom
	...
	Function Init
		...
		this.oDelayedCode = CreateObject('Collection')
	Endfunc
	...
	Function interpret		
		...
		lnIndex = this.oDelayedCode.GetKey(loControl.name)
		loCode = this.oDelayedCode.Item(lnIndex)
		Return loCode.call(This.oInterpreter, CreateObject('Collection'))
	EndFunc
Enddefine
...
Define Class Resolver As Custom
	...
	Function visitClassNodeStmt(toStmt)
		...
		For Each loMethod In toStmt.oMethods
			...
			This.resolveFunction(loMethod, lnDeclaration, false)
		Endfor
	...
	Function visitFunctionNodeStmt(toStmt)
		...
		This.resolveFunction(toStmt, FT_FUNCTION, false)
	...
	Function visitFunctionArrowExpr(toExpr)
		This.resolveFunction(toExpr, FT_FUNCTION, true)

		Return .Null.
	Endfunc	
		
	Function visitFunctionExpr(toExpr)
		This.resolveFunction(toExpr, FT_FUNCTION, false)
	...
	Hidden Function resolveFunction(toFunction, tnType, tlisArrowFunction)
		...
		For Each toParam In toFunction.oParams
			If tlisArrowFunction
				This.Declare(toParam.oName)
				This.Define(toParam.oName)
			else
...
Define Class Context As Custom
	...
	Function Accept(toVisitor)
		Return toVisitor.visitContextExpr(This)
	Endfunc
Enddefine
...
Define Class Interpreter As Custom
	...
	Function Init
		with this
			...
			.oGlobals.Define("Call", Createobject("BuiltinCall"))
			.oGlobals.Define("Query", Createobject("BuiltinQuery"))
			.oGlobals.Define("Add", Createobject("BuiltinAdd"))
		endwith
	Endfunc
	...
	Function visitPrintStmt(toStmt)
		...
		If Type('loValue') == 'O' and Type('loValue.Tag') == 'C' and loValue.Tag == 'Grouping'
			For each loVal in loValue
				Messagebox(This.stringify(loVal))
			EndFor
		Else
	...
	Function visitCallExpr(toExpr)
		...
		*Return this.executeFunction(loFunction)
	...
	Function visitGetExpr(toExpr)
		...
		If Type('loObject') == 'O'
			If loObject.Class == 'Runtimeinstance' or loObject.ParentClass == 'Runtimeinstance'
				Return loObject.Get(toExpr.oName)
			Else
				try
					Local loResult, lcMacro
					loResult = .null.
					If Type('loObject.baseclass') == 'C' and loObject.baseclass == 'Collection'
						lcMacro = "loObject.Item(loObject.Getkey('" + toExpr.oName.lexeme + "'))"
					Else
						lcMacro = "loObject." + toExpr.oName.lexeme
					EndIf
					loResult = &lcMacro
				Catch to loEx
					_Screen.FoxScript.throwError('RuntimeError', 'token', toExpr.oName, loEx.Message)
				EndTry
				Return loResult
	...
	Hidden function executeFunction(toFunction)
		Local loFunction, loArguments
		loFunction = toFunction
